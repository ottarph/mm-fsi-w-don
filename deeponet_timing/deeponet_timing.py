...
def time_deeponet(problem_dict: dict) -> None:
    ...
    evaluation_points = torch.tensor(V_scal.tabulate_dof_coordinates(), dtype=torch.float32)
    ...
    for _ in range(N):
        ...
        uh_torch = torch.tensor(u_out, dtype=torch.float32)
    end = timer()
    A = (end-start)/N
    print(f"A: {A:.2e}")
    ...
    with torch.no_grad():
        ...
        start = timer()
        for _ in range(N):
            corr_np = corr_t.detach().double().numpy()
...
def main():
    ...
    parser.add_argument("--save-to-file", default=Path("output/data/deeponet_timing.npy"), type=Path)
