...
Path("output/fsi_run_bih").mkdir(parents=True, exist_ok=True)

# save options
FSI_param["save_data_dir"] = str(Path("output/fsi_run_bih/data"))
FSI_param["save_state_dir"] = str(Path("output/fsi_run_bih/state"))
FSI_param["save_snapshots_dir"] = str(Path("output/fsi_run_bih/snapshots"))
...
fsisolver = solver.FSIsolver(mesh, boundaries, domains, params, FSI_param, extension_operator, warmstart=WARMSTART) #warmstart needs to be set to False if warmstart-checkpoint not available
